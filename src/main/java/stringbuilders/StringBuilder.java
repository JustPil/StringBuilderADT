...
public class StringBuilder {
    ...
    /**
     * append Overloaded append method adds a short to the StringBuilder. The short may be negative or positive, one or
     * many digits.
     * @param s The short value to add.
     */
    public void append(short s) {
        String shortStr = Short.toString(s);
        if(((double)((numElements + shortStr.length()) / capacity) >= LOAD_THRESHOLD)) {
            resize();
        }
        for(int i = 0; i < shortStr.length(); i++) {
            array[numElements++] = shortStr.charAt(i);
        }
    }
